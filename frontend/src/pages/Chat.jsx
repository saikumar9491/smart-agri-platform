            ...
            <>
              ...
              <div className="flex-1 relative min-h-0 bg-white">
                                    ...
                                    {msg.attachment && (
                                      <div className="mt-2 rounded-xl overflow-hidden border border-white/20">
                                        {msg.attachmentType === 'image' ? (
                                          <img src={msg.attachment} alt="attachment" className="max-h-64 w-full object-cover" />
                                        ) : (
                                          <div className="bg-white/10 p-3 flex items-center gap-3">
                                            <FileText className="h-8 w-8" />
                                            <div className="flex-1 min-w-0">
                                              <p className="text-xs truncate font-medium">Document</p>
                ...
                {!isAtBottom && messages.length > 5 && (
                  <button 
                    ...
                    className="absolute bottom-24 right-6 p-3 bg-white border border-slate-100 shadow-xl rounded-full text-blue-600 hover:bg-slate-50 transition-all duration-300 animate-in zoom-in slide-in-from-bottom-4 z-[99]"
                  >
              ...
              {isPartnerTyping && (
                <div className="px-6 py-2">
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 text-slate-500 text-xs animate-pulse">
              ...
              <div className="fixed bottom-0 left-0 right-0 md:relative bg-white p-3 md:p-4 border-t border-slate-100 pb-safe z-50">
                  ...
                  <div className="mb-2 p-2 bg-slate-50 rounded-lg flex items-center justify-between animate-in slide-in-from-bottom-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-[10px] text-blue-600 font-bold uppercase">Replying to {replyingTo.sender?.name || "User"}</p>
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2 md:gap-3">
                  <div className="flex items-center gap-1">
                    <button type="button" onClick={() => handleFeatureComingSoon("Camera")} className="p-2 text-blue-600 hover:bg-blue-50 rounded-full transition-all">
                  ...
                  <div className="flex-1 relative">
                    <input
                      type="text"
                      className="w-full bg-slate-100 border-none rounded-2xl py-2.5 px-4 text-sm focus:ring-2 focus:ring-blue-500/20 placeholder:text-slate-400"
                      placeholder="Message... (v2.2)"
                      value={newMessage}
                    ...
                    <div className="absolute right-2 top-1.5 flex items-center gap-1">
                      <button type="button" onClick={startRecording} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors">
                        <Mic className="h-4 w-4 md:h-5 md:w-5" />
                      </button>
                      <button type="button" onClick={triggerImageUpload} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors">
                        <Image className="h-4 w-4 md:h-5 md:w-5" />
                      </button>
                      <button type="button" onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors">
                  ...
                  <button
                    ...
                    className={cn(
                      "font-bold text-sm px-4 py-2 transition-all duration-300 rounded-full",
                      newMessage.trim() 
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900 flex flex-col items-center justify-center animate-in fade-in duration-500">
          <div className="relative w-full h-full flex flex-col">
            <div className="flex-1 bg-black relative flex items-center justify-center overflow-hidden">
              {callType === 'video' || call.type === 'video' ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-green-400 animate-pulse tracking-widest text-xs font-bold uppercase">Voice Call Active</p>
                </div>
              )}
              {!callAccepted && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center backdrop-blur-sm">
                  ...
                  <p className="text-white font-bold tracking-widest animate-pulse uppercase">Connecting...</p>
                </div>
              )}
            </div>
            ...
            <div className="absolute bottom-12 left-0 w-full flex justify-center gap-8 items-center px-6">
              ...
              <button onClick={handleEndCall} className="h-16 w-16 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 transition-all shadow-xl shadow-red-500/40 active:scale-95">
      ...
      {call.isReceivingCall && !callAccepted && (
        <div className="fixed inset-0 z-[110] bg-black/80 backdrop-blur-xl flex flex-col items-center justify-center p-8 animate-in zoom-in duration-300">
          <div className="flex flex-col items-center text-center max-w-sm w-full bg-slate-900 p-10 rounded-[40px] border border-white/10 shadow-2xl">
            ...
            <p className="text-blue-400 text-sm font-bold uppercase tracking-widest mb-10">Incoming {call.type} Call...</p>
            <div className="flex gap-10 w-full justify-center">
              <button onClick={handleEndCall} className="h-16 w-16 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 shadow-lg active:scale-90">
                <X className="h-8 w-8" />
              </button>
              <button onClick={handleAnswerCall} className="h-16 w-16 rounded-full bg-green-500 text-white flex items-center justify-center hover:bg-green-600 shadow-lg shadow-green-500/30 animate-bounce active:scale-90">
