          ...
          <div className="flex flex-col gap-6 py-4 px-2">
            ...
            <div className="flex items-center justify-between w-full">
              <Link to="/" className="flex items-center gap-2">
                ...
                <span className="font-black text-xl tracking-tighter bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent drop-shadow-sm" style={{ fontFamily: "'Outfit', sans-serif" }}>Farm</span>
              </Link>

              <div className="flex items-center gap-2">
                {user?.role === 'admin' && (
                  <Link to="/app/admin" className="h-8 w-8 rounded-full glassmorphic flex items-center justify-center text-rose-400 border border-rose-500/20 shadow-[0_0_15px_rgba(244,63,94,0.2)]">
                    <ShieldCheck className="h-4 w-4" />
                  </Link>
                )}

                {/* Search Trigger (Icon Only) */}
                <button className="h-8 w-8 rounded-full glassmorphic flex items-center justify-center text-white/70 hover:text-white">
                ...
                <div className="relative">
                  <button
                    ...
                    className="h-8 w-8 rounded-full glassmorphic flex items-center justify-center text-white relative"
                  >
                    <Bell className="h-4 w-4" />
                    {data.notifications?.length > 0 && (
                      <span className="absolute top-0 right-0 h-2 w-2 rounded-full bg-rose-500 border-2 border-[#2F4F4F]"></span>
                    )}
                  </button>
                  {showNotifications && (
                    <>
                      <div className="fixed inset-0 z-[90]" onClick={() => setShowNotifications(false)} />
                      <div className="fixed top-[70px] left-4 right-4 bg-slate-900/95 backdrop-blur-3xl border border-slate-700/50 rounded-[2rem] shadow-2xl z-[100] overflow-hidden">
                        <div className="p-5 border-b border-white/10 bg-slate-800/40 text-white flex justify-between items-center">
                          ...
                          <span className="flex h-5 w-5 items-center justify-center rounded-full bg-rose-500/20 text-[9px] font-black text-rose-400">
                            {data.notifications?.length || 0}
                          </span>
                        </div>
                              ...
                              <div key={n._id} className="p-4 border-b border-white/5 hover:bg-white/5 flex gap-3 text-sm">
                                <div className="mt-1">
                                  {n.type === 'warning' ? <AlertTriangle className="h-3 w-3 text-amber-500" /> :
                                    n.type === 'success' ? <CheckCircle className="h-3 w-3 text-green-500" /> :
                ...
                {/* Profile */}
                <Link to="/app/profile" className="h-8 w-8 rounded-full bg-green-500/20 border-2 border-green-400 overflow-hidden shadow-[0_0_15px_rgba(74,222,128,0.2)]">
                  {user?.profilePic ? (
                    <img src={resolveImageUrl(user.profilePic, '')} className="h-full w-full object-cover" alt="" />
                  ) : (
                    <div className="h-full w-full flex items-center justify-center text-white font-black text-[9px]">
