...
import { Leaf, Loader2, Mail, ShieldCheck, Lock, ArrowLeft, ArrowRight, CheckCircle2, Eye, EyeOff } from 'lucide-react';
      ...
      <div className="w-full max-w-md space-y-6 rounded-3xl bg-white p-10 shadow-2xl border border-slate-100">
        <div className="text-center">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 shadow-lg shadow-amber-500/25">
            {step === 3 ? <CheckCircle2 className="h-8 w-8 text-white" /> : <Lock className="h-8 w-8 text-white" />}
          </div>
        ...
        {step === 1 && (
          <form onSubmit={handleSendOtp} className="space-y-5">
            ...
            >
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><span>Send Reset Code</span> <ArrowRight className="h-4 w-4" /></>}
            </button>
          </form>
        )}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="password"
                  required
                  className="block w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 pl-11 text-slate-900 focus:border-amber-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all sm:text-sm"
